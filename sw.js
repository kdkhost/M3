...
// Lista de assets para pré-cache (inclui as duas páginas)
const ASSETS_TO_PRECACHE = [
    '/',
    '/erro/index.html', // Página para erro 404
...
// Tempos de cache (em segundos) - **Observação: Estas variáveis não são usadas nas funções de estratégia**
const MAX_AGE = {
    css: 86400 * 7, // 7 dias
    js: 86400 * 7, // 7 dias
...
// Páginas HTML (Network First com fallback para cache e tratamento de erro 404 e offline)
    ...
    event.respondWith(
        (async () => {
            try {
                ...
                // Se a resposta for um erro 404, retorna a página de erro 404
                if (networkResponse.status === 404) {
                    return caches.match('/erro/index.html');
                }

                // Atualiza o cache (apenas se a resposta for bem-sucedida)
                if (networkResponse.ok) {
                    const clone = networkResponse.clone();
                ...
                // Em caso de falha na rede (usuário offline), retorna a página offline do cache
